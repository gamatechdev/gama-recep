...
export const AppointmentModal: React.FC<AppointmentModalProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      ...
      if (initialData.unidade_id) {
        setAppointmentData((prev) => ({
          ...prev,
          unidade: initialData.unidade_id!,
        }));
      }
    }
  }, [initialData]);
    ...
    if (unitsLoadedRef.current) {
      ...
      if (initialData?.unidade_id) {
        const u = unidades.find(
          (unit) => unit.id.toString() === initialData.unidade_id,
        );
        if (u) setUnitSearchTerm(u.nome_unidade);
      }
      // Else if we have a name
      else if (initialData?.empresa) {
        const matchedUnit = unidades.find(
          (u) =>
            u.nome_unidade &&
            u.nome_unidade
              .toLowerCase()
              .includes(initialData.empresa!.toLowerCase()),
        );
        if (matchedUnit) {
          setAppointmentData((prev) => ({
            ...prev,
            unidade: matchedUnit.id.toString(),
          }));
  ...
  const fetchInitialData = async () => {
    // Busca colaboradores com o nome do cargo via join na tabela 'cargos'
    const { data: colabs } = await supabase
      .from("colaboradores")
      .select(
        "id, nome, cpf, data_nascimento, sexo, setor, cargo, cargos(nome)",
      );
    const { data: units } = await supabase
      .from("unidades")
      .select("id, nome_unidade");

    // Cast como any[] pois o select com join (cargos) retorna formato expandido
    if (colabs) setColaboradores(colabs as any[]);
    if (units) {
      setUnidades(units);
  ...
  const handleSelectColab = (colab: any) => {
    ...
    setColabFormData({
      ...
      sexo: colab.sexo || "M",
      setor: colab.setor || "",
      // Prioriza o nome do cargo (tabela cargos) sobre o ID numérico
      funcao: colab.cargos?.nome || colab.setor || "",
    });
    setColabSearchTerm("");
  };
  ...
  const filteredExams = EXAMES_LIST.filter(
    (exame) =>
      exame.nome &&
      exame.nome.toLowerCase().includes(examSearchTerm.toLowerCase()),
  );

  const filteredColabs = colaboradores.filter(
    (c) =>
      (c.nome &&
        c.nome.toLowerCase().includes(colabSearchTerm.toLowerCase())) ||
  ...
  const filteredUnits = unidades.filter(
    (u) =>
      u.nome_unidade &&
      u.nome_unidade.toLowerCase().includes(unitSearchTerm.toLowerCase()),
  );
  ...
  const handleNotifyYes = async () => {
    ...
    const { data, error } = await supabase.from("chat_tags").select("*");
  ...
  const handleSendMessage = async () => {
    ...
    const formattedDate = new Date(savedContext.data).toLocaleDateString(
      "pt-BR",
    );
    const examsListString = savedContext.exames
      .map((e: string) => `- ${e}`)
      .join("\n");
    ...
    try {
      const response = await fetch(
        "https://api.z-api.io/instances/3E8112AFC26DD1A98FF7B2116B9188C4/token/A5112854C9B41DACC9EA5B85/send-text",
        {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const unitId = appointmentData.unidade
        ? parseInt(appointmentData.unidade)
        : null;

      // Cargo logic: find or create cargo based on colabFormData.funcao
      let finalCargoId = null;
      if (colabFormData.funcao?.trim()) {
        const { data: existingCargo } = await supabase
          .from("cargos")
          .select("id")
          .ilike("nome", colabFormData.funcao.trim())
          .maybeSingle();
        if (existingCargo) {
          finalCargoId = existingCargo.id;
        } else {
          const { data: newCargo, error: cErr } = await supabase
            .from("cargos")
            .insert({ nome: colabFormData.funcao.trim(), ativo: true })
            .select()
            .single();
          if (cErr) throw cErr;
      ...
      if (mode === "new") {
          ...
          .insert([
            {
              ...
              setor: colabFormData.setor?.trim() || null,
              cargo: finalCargoId,
              unidade: unitId,
            },
          ])
        ...
        if (!finalColabId)
          throw new Error("Selecione um colaborador existente.");
      ...
      const pdfPayload = {
        ...
        paciente: {
          ...
          cpf: colabFormData.cpf,
          funcao:
            colabFormData.funcao || colabFormData.setor || "Não informado",
          sexo: colabFormData.sexo,
      ...
      console.log(
        "DEBUG: Payload enviado para Geração de PDF (Modal):",
        pdfPayload,
      );
      ...
      console.log(
        "DEBUG: Payload de INSERÇÃO do Agendamento (Supabase - Modal):",
        agendamentoPayload,
      );
      ...
      setMessage({
        type: "error",
        text: err.message || "Erro ao realizar agendamento.",
      });
    } finally {
      setLoading(false);
    }
      ...
      <div className={containerClasses}>
        ...
        {isPublic && (
          <div className="flex flex-col items-center mb-6">
            ...
            <h2 className="text-3xl font-bold text-slate-800 dark:text-white tracking-tight text-center">
              Autoagendamento
            </h2>
          </div>
        )}
        ...
        {!isPublic && (
          <h2 className="text-3xl font-bold text-slate-800 dark:text-white mb-8 tracking-tight">
            Novo Agendamento
          </h2>
        )}
        ...
        <div className="bg-white dark:bg-[#1c1c1e] rounded-xl p-2">
          {message && (
            <div
              className={`mb-6 p-4 rounded-xl text-sm font-medium flex items-center gap-2 ${message.type === "success" ? "bg-green-50 dark:bg-green-500/10 text-green-700 dark:text-green-400" : "bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400"}`}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {mode === "existing" && !selectedColabId && (
              <div className="relative mb-8">
                <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wide ml-1">
                  Buscar Colaborador
                </label>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                    <svg
                      className="h-5 w-5 text-gray-400 group-focus-within:text-[#04a7bd] transition-colors"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
            ...
            <div className="space-y-6">
              <h3 className="text-sm uppercase tracking-wider text-slate-500 dark:text-slate-400 font-bold border-b border-gray-100 dark:border-white/5 pb-2">
                  ...
                  <div className="relative group">
                    ...
                    {!initialData?.unidade_id && (
                      <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        <svg
                          className="h-4 w-4 text-gray-400 group-focus-within:text-[#04a7bd]"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white dark:bg-[#2c2c2e] rounded-[24px] p-6 max-w-sm w-full shadow-2xl scale-100 border border-white dark:border-white/5">
              ...
              <p className="text-center text-slate-500 dark:text-slate-400 mb-6 text-sm">
                Deseja enviar uma mensagem de confirmação para a empresa via
                WhatsApp?
              </p>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 font-semibold text-white bg-[#04a7bd] hover:bg-[#149890] rounded-xl transition-colors shadow-lg shadow-[#04a7bd]/30"
                >
                  Sim
                </button>
              </div>
                ...
                {filteredContacts.map((contact) => (
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      <p className="font-bold text-slate-900 dark:text-white group-hover:text-[#04a7bd] transition-colors truncate">
                        {contact.chatname}
                      </p>
        ...
        {showConfirmSendModal && selectedContact && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white dark:bg-[#2c2c2e] rounded-[24px] p-6 max-w-sm w-full shadow-2xl border border-white dark:border-white/5">
              ...
              <p className="text-center text-slate-500 dark:text-slate-400 mb-6 text-sm">
                Enviar mensagem automática para{" "}
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 animate-fadeIn">
            <div className="bg-white dark:bg-[#2c2c2e] rounded-[24px] p-8 max-w-sm w-full shadow-2xl text-center border border-white dark:border-white/5 animate-scaleIn">
              ...
              <p className="text-slate-500 dark:text-slate-400 mb-8">
                Seu pré-agendamento foi realizado com sucesso. Aguarde a
              ...
              <button
                ...
                }} // Simple reload to clear/reset public view
